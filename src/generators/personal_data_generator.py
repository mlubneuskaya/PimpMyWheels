...
class PersonalDataGenerator:
    def __init__(self, names, female_surnames, male_surnames, min_age=18, avg_age=30, max_age=80, age_scale=15):
        self.names = names
    ...
    def get_unique_name_surname(self):
        def get_name_surname():
            ...
            if sex == "F":
                surname = self.female_surnames.sample(n=1, weights="frequency").surname.iloc[0]
            else:
                surname = self.male_surnames.sample(n=1, weights="frequency").surname.iloc[0]
            return name, surname
        ...
        while (
            ...
            unique_name, unique_surname = get_name_surname()
        self.name_surname_pairs.add(f"{unidecode(unique_name)}{unidecode(unique_surname)}")
    ...
    def get_address(self, customer=False):
        if customer:
            city = random.choices([self.city, mimesis.Address(locale=mimesis.Locale.PL).city()], weights=[.8, .2])[0]
        else:
            city = self.city
    ...
    def get_birth_date(self, day):
        a, b = (self.min_age - self.avg_age) / self.age_scale, (self.max_age - self.avg_age) / self.age_scale
        age = scipy.stats.truncnorm.rvs(a=a, b=b, loc=self.avg_age, scale=self.age_scale)
